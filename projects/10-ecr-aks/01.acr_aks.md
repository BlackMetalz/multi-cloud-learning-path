...
Build local because `az acr build` will not work on trial subscription. And Why I used to build multiple platform? becaue I'm using Mac to build, not Ubuntu (linux/amd64)
```bash 
az acr login -n $ACR_NAME
# Login Succeeded
docker buildx build \
...
Continue to [Devops Azure](README.md)